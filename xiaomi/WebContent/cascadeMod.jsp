...
<body>
	<form action="AddressServlet" method="post" class="smart-green">
		<h1>修改收货信息</h1>
			<input type="hidden" name="operate" value="modifyAddress" />
			<%
				String aid = request.getParameter("aid");
				String uid = request.getParameter("uid");
				String name = new String(request.getParameter("name").getBytes("ISO-8859-1"), "UTF-8");
				String contact = request.getParameter("contact");
				System.out.println("为了修改收货信息，获取到地址的收货人：" + name);
			%>
			<input id="aid" type="hidden" name="aid" value=<%= aid %> />
			<input id="uid" type="hidden" name="uid" value=<%= uid %> />
            <input id="name" type="text" name="name" value=<%= name %> />
            <input id="phone" type="text" name="contact" value=<%= contact %> /> 
...
     		<label><span>地址类型</span></label>
    		...
    		<br><br><br><br>
       		<input type="button" class="button" value="确认修改" onclick="tijiao()"/>
	</form>
</body>
</html>